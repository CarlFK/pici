...
function PiStatus(PiID) {
  ...
  function updateStatus(status, text) {
    const badge = document.getElementById("status-text-" + PiID);
    const dot = document.getElementById("status-dot-" + PiID);
    if (badge && dot) {
  ...
  window.toggleLogs = toggleLogs; // Make globally available

  function connect() {
    updateStatus("disconnected", "Connecting...");

    const logSocket = new WebSocket(
      "WSS://" + window.location.host + "/ws/pistat/" + "pi" + PiID + "/"
    );
    ...
    logSocket.onmessage = function (e) {
      ...
      if (data.message == "piview: ssh ssh server started.") {
        console.log("ssh!");
        wssh_connect();
        updateStatus("connected", "SSH Ready");
      }
      if (data.status == "cam") {
    ...
    function check_status() {
      logSocket.send(JSON.stringify({ message: "checking status: " + PiID }));
    ...
    document.getElementById("reset" + PiID).onclick = function (e) {
      ...
      logSocket.send(JSON.stringify({ message: "reset: " + PiID }));
  ...
  document.getElementById("refresh-video-player" + PiID).onclick = function (
    e
  ) {
