...
sub image_list {
    ...
    my $kernel_list_filename = '/var/lib/dpkg/info/raspberrypi-kernel.list';
    open(kernel_list_file, '<', $kernel_list_filename) or die $!;
    while(<kernel_list_file>) {
        next if $_ !~ /lib\/modules\/[^\/]*\/kernel$/;
        next if $_ !~ /(\d[^\/]*)/;
        push(@results, [$1, $_]);
    }
