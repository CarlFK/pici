---
eth_local_mac_address: 00:25:90:22:c4:91
eth_local_address: 10.21.0.1
eth_local_netmask: 24

eth_uplink_mac_address: 00:25:90:22:c4:90
eth_uplink_static: true
eth_uplink_static_address: 76.227.131.147
eth_uplink_static_netmask: 26
eth_uplink_static_gateway: 76.227.131.254
eth_uplink_dns_server: 8.8.8.8

dhcp_range: 10.21.0.128,10.21.0.254,6h

firewall_internal_networks: [10.21.0.0/24]

pib_network: 10.21.0

user_name: videoteam

time_zone: America/Chicago

conference_name: pib
room_name: catwalk

domain_name: ps1.fpgas.mithis.com
letsencrypt_account_email: carl@NextDayVideo.com

# snmpget -v 3 -u  -l authPriv -a MD5 -x DES -A wordpass -X wordpass -c pib \

switch:
    # Netgear #2 GS728TPP mac: "b0:39:56:88:22:4c"

    # HP https://support.hpe.com/hpesc/public/docDisplay?docId=c02596727
    # pethMainPseOperStatus .1.3.6.1.2.1.105.1.3.1.1.3

    # James Netgear:
    # http://kbserver.netgear.com/products/FS728TPv2.asp
    # FS728TP v2
    mac: "A0:21:B7:AF:4E:05"
    oid: "iso.3.6.1.4.1.4526.11.16.1.1.1.3.1"
    host: "10.21.0.200"


    # pi things
    nos:
    - { port:  2, mac: "b8:27:eb:2f:5d:08", sn: 042f5d08, loc: "front 1", cable_color: "blue" }
    - { port:  3, mac: "dc:a6:32:05:32:45", sn: f1b7bb5a, loc: "front 2", cable_color: "blue"}
    - { port:  4, mac: "e4:5f:01:64:76:05", sn: e897e1d3, loc: "front 3", cable_color: "blue" }
    - { port:  5, mac: "b8:27:eb:d4:f1:74", sn: 9ed4f174, loc: "front 4", cable_color: "blue" }
    - { port:  7, mac: "b8:27:eb:33:51:27", sn: 48335127, loc: "front 5", cable_color: "blue" }
    - { port:  9, mac: "b8:27:eb:a3:51:b4", sn: 8da351b4, loc: "front 6", cable_color: "blue" }
    - { port: 11, mac: "b8:27:eb:51:01:df", sn: 265101df, loc: "front 7", cable_color: "blue" }
    - { port: 13, mac: "b8:27:eb:68:fc:e7", sn: f168fce7, loc: "front 8", cable_color: "blue" }
    - { port: 19, mac: "b8:27:eb:69:79:a0", sn: 426979a0, loc: "back 8", cable_color: "blue" }
    - { port: 21, mac: "b8:27:eb:5f:de:85", sn: 7d5fde85, loc: "back 7", cable_color: "blue" }
    - { port: 23, mac: "b8:27:eb:0c:f8:43", sn: 9b0cf843, loc: "back 6", cable_color: "blue" }
      # - { port: ?, mac: "b8:27:eb:5a:26:5b", sn: 035a265b }


    SNMP_SWITCH_SECURITY_LEVEL: authPriv
    SNMP_SWITCH_AUTH_PROTOCOL: MD5
    SNMP_SWITCH_PRIV_PROTOCOL: DES

    SNMP_SWITCH_USERNAME: !vault |
          $ANSIBLE_VAULT;1.1;AES256
          
          3536
    SNMP_SWITCH_AUTHKEY: !vault |
          $ANSIBLE_VAULT;1.1;AES256
          
          6466
    SNMP_SWITCH_PRIVKEY: !vault |
          $ANSIBLE_VAULT;1.1;AES256
          
          3333

pi_pw: !vault |
          $ANSIBLE_VAULT;1.1;AES256
          
          6135


